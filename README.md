...
## Описание проекта
**BusRouteVisualization** — это приложение на C# с использованием Windows Forms, которое позволяет визуализировать маршруты общественного транспорта на карте. Приложение поддерживает:
- Создание новых маршрутов.
- Добавление точек на маршруты.
- Отображение маршрутов на `PictureBox`.
- Анимацию движения автобуса по маршруту.

Все данные о маршрутах сохраняются в файл `routes.json` для последующего использования.
...
- Скачайте или склонируйте проект с помощью Git:
  ```
...
- **Динамический путь (опционально)**:
  - Можно настроить сохранение в папке пользователя (например, `%AppData%\BusRouteVisualization\routes.json`) путём изменения кода:
    ```csharp
    private static readonly string FilePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "BusRouteVisualization", "routes.json");
    ```
...
### Описание кода
#### Класс **Form1**
- **Назначение**: Основной класс формы, содержащий всю логику работы приложения.
- **Переменные**:
...
#### Класс **Prompt**
- **Назначение**: Статический класс для отображения диалогового окна ввода текста (например, для названия точки).

#### Класс **BusRoute**
- **Назначение**: Представляет маршрут общественного транспорта.
- **Свойства**:
...
#### Класс **RoutePoint**
- **Назначение**: Представляет точку на маршруте.
- **Свойства**:
